<template>
  <aside>
    <RouterLink :to="{ name: 'Home' }" title="Home"
                class="logo-link lg:block" :class="{ hidden: $router.currentRoute.value.name !== 'Home' }"
    >
      <YLogo icon animated />
    </RouterLink>
    ...
    <SpeedDial :model="filteredItems" :tooltip-options="{position: 'right'}" mask
               class="speed-dial-menu"
               show-icon="pi pi-bars" hide-icon="pi pi-times"
...
<script lang="ts">
  ...
  export default defineComponent({
        ...
        items: [
          {
            ...
            command: () => this.$router.push({ name: "Moderation", params: { roomId: this.$yardAPI.store.roomId } }),
            visible: () => !!this.$yardAPI.store.roomId && this.$router.currentRoute.value.name !== "Moderation",
          },
          {
            icon: "pi pi-desktop",
            label: "Go to Yard",
            command: () => this.$router.push({ name: "Yard", params: { roomId: this.$yardAPI.store.roomId } }),
            visible: () => !!this.$yardAPI.store.roomId && this.$router.currentRoute.value.name !== "Yard",
          },
          {
            icon: "pi pi-window-maximize",
            label: "Enter Fullscreen",
            visible: () => !!this.$yardAPI.store.roomId && screenfull.isEnabled && !this.isFullscreen,
    ...
    computed: {
      filteredItems(): any[] {
        return this.items.filter((item: any) => item.visible ? item.visible() : true);
      },
    },
    mounted() {
