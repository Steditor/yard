<template>
  <g class="player" ref="group" :style="playerStyle" :class="playerClass">
    ...
    <rect
      ...
      v-if="pixelId === $yardAPI.store.pixelHighlight?.pixelId"
      :x="-pixelSize / 2"
      :y="-pixelSize / 2"
      :width="pixelSize"
      :height="pixelSize"
  ...
  export default defineComponent({
    ...
    computed: {
      ...
      labelYPosition(): number {
        return (
          (0.5 * this.pixelSize + 2 * this.textHeight) * this.labelYDirection
        );
      },
      labelYDirection(): number {
        return this.pixel.y / this.$yardAPI.store.settings.canvasHeight < 0.15
          ? 1
          : -1;
      },
      labelPath(): string {
        return `M0 ${(this.textHeight / -2) * this.labelYDirection} V${-this
          .labelYPosition} L${-this.labelXPosition} ${-this.labelYPosition}`;
      },
    },
    methods: {
      selectPixel() {
