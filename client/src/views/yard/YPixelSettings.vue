<template>
  <ColorPicker
    v-model="pixelColor"
    @pointerover="selectAndHighlightPixel"
    @pointerout="unhighlightPixel"
    class="pixel-color-picker"
    :class="{ active }"
  />
</template>
    ...
    watch: {
      active(): void {
        if (this.active && this.overlayScrollbars) {
          this.overlayScrollbars.scroll(
            { el: this.$el, scroll: "ifneeded", block: "nearest" },
            500,
          );
        }
      },
