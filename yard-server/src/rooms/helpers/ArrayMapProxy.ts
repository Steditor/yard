...
export default class ArrayMapProxy<T> implements Pick<Array<T>, "length" | "splice" | "indexOf"> {
  ...
  splice(start: number, deleteCount?: number, ...items: T[]): T[] {
    ...
    const lastUpdateIndex = deleteCount !== items.length ? this.array.length : start + deleteCount;
